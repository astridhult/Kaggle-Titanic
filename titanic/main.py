...
Fares_with_const = sm.add_constant(pandas_df['Fare'])

# Fit the model
model = sm.OLS(pandas_df['Survived'], Fares_with_const)
results = model.fit()
st.write(results.summary())

coefficients = np.array(results.params)
Survived_fitted = Fares_with_const @ coefficients

pandas_df['Survived_fitted'] = Survived_fitted

# st.dataframe(pandas_df)

# Survived_measured = df['Survived']

# df_linreg = pl.DataFrame({'Fares': Fares_with_const, 'Survived': Survived_measured , 'Survived_fitted': Survived_fitted})
